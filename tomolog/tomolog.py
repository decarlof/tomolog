...
def show_tomolog(args):
    ...
    if os.path.isfile(fname): 
        ...
        log.info('All tomolog data available in the HDF5 raw data file: start')
        log.info(all_tomolog)
        log.info('All tomolog data available in the HDF5 raw data file: end')
    tomolog, year_month, pi_name = extract_meta(args)
    if tomolog != None:
        log.info('All tomolog data extracted from the HDF5 raw data file:')
        print(tomolog)
...
def extract_dict(fname, list_to_extract, index=0):
    ...
    tomolog = dxreader.read_dx_meta(fname) 
    # print(tomolog)
    try: 
        dt = datetime.datetime.strptime(tomolog['start_date'][0], "%Y-%m-%dT%H:%M:%S%z")
        ...
        year_month = '2020-02'
    pi_name = tomolog['experimenter_name'][0]

    # compact full_file_name to file name only as original data collection directory may have changed
    tomolog['full_file_name'][0] = os.path.basename(tomolog['full_file_name'][0])

    # sub_dict = {k:v for k, v in tomolog.items() if k in list_to_extract}
    sub_dict = {(('%3.3d' % index) +'_' + k):v for k, v in tomolog.items() if k in list_to_extract}
...
def extract_meta(args):
    ...
    pi_name = 'unknown'
    if os.path.isfile(fname): 
        tomolog_dict, year_month, pi_name = extract_dict(fname, list_to_extract)
        # print (tomolog_dict, year_month, pi_name)
    ...
    df = pd.DataFrame.from_dict(tomolog_dict, orient='index', columns=('value', 'unit'))
